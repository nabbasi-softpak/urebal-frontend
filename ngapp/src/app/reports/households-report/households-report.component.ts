...
@Component({
  selector: 'app-households-report',
  templateUrl: './households-report.component.html',
  styleUrls: ['./households-report.component.css']
})
export class HouseholdsReportComponent implements OnInit, AfterViewInit {

  @ViewChild(UrebalGridComponent) houseHoldGrid :UrebalGridComponent;
  householdAccounts : number = 0;
  ...
  ngAfterViewInit()
  {
    /*** Make sure grid binding is completed to get Total number of Households ***/
    this.houseHoldGrid.jqxGrid.onBindingcomplete.subscribe( grid =>
      {
        this.householdAccounts  = this.houseHoldGrid.getRows().length;
      }
    );
  ...
  printReport(){
    ...
    // let gridContent = this.houseHoldGrid.jqxGrid.host.jqxGrid('exportdata','html');
    //
    // let pageContent =
    //   '<!DOCTYPE html>' +
    //   '<html>' +
    ...
    let gridContent = this.houseHoldGrid.jqxGrid.host.jqxGrid('exportdata', 'html', null);
    ...
    setTimeout(() => {
      try {
        ...
        //this.houseHoldGrid.refreshGrid();
      }
      catch (error) {
        console.log(error)
      }
  ...
  downloadReport() {
    ...
    let csv =
      '"Number of Households:",' + this.householdAccounts +
      '\n\n' +
      this.houseHoldGrid.jqxGrid.host.jqxGrid('exportdata', 'csv', null);
    let blob = new Blob([csv], {type: 'application/csv'});
    saveAs(blob,'Households-Report.csv');
    //this.houseHoldGrid.refreshGrid();
  }
