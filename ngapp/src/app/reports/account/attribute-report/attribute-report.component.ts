...
@Component({
  ...
  templateUrl: './attribute-report.component.html'
})
export class AttributeReportComponent implements OnInit {
  ...
  getReport(portfolio) {
    ...
    this.accountService.get("attributesummary/getAssetAllocationReport/"+portfolio.portfolioId).subscribe(result=> {
      if(result.code == 200) {
        this.attributeReport = result.responsedata;
        this.errMsg = '';
      ...
      this.attributeReportGrid.refreshGrid(this.attributeReport);
    });
  ...
  printme() {
    this.disableBtns = true;
    this.attributeReportGrid.treeGridSettings.exportSettings.fileName = null;
    let gridContent = this.attributeReportGrid.jqxTreeGrid.exportData('html');
  ...
  exportme() {
    this.attributeReportGrid.treeGridSettings.exportSettings.fileName = null;
    //this.attributeReportGrid.jqxTreeGrid.exportData('csv');

    let gridCSV = this.attributeReportGrid.jqxTreeGrid.exportData('csv');
    let blob = new Blob([this.generateCSV(gridCSV)], {type: 'text/plain'});
    saveAs(blob,'Attributes-Summary.csv');
  }
